use crate::{Pattern, Show, Term, Type, error::Spanned, parser::Parsed};
use std::ops::Deref;

impl<T> Show for Spanned<T>
where
    T: Show,
{
    fn show(&self, ctx: &crate::Context, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        self.item.show(ctx, fmt)
    }
}
    ...
    fn show(&self, ctx: &crate::Context, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            match self {
                ...
                Term::Symbol(s) => format!("(Symbol {})", s.item),
            }
        )
    }
}
    ...
    fn show(&self, ctx: &crate::Context, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            match self {
                ...
                Pattern::Symbol(s) => format!("(Symbol {})", s.item),
            }
        )
    }
}
